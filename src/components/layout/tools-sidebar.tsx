...
export function ToolsSidebar({ activeHref }: { activeHref?: string }) {
  ...
  return (
    <>
      {/* Nút hamburger - hình vuông nhỏ gọn, cố định góc trên trái, chỉ hiện dưới breakpoint lg */}
      <button
        type="button"
        onClick={() => setOpen(true)}
        aria-label="Open tools menu"
        className="fixed right-4 top-2 z-40 flex h-10 w-10 items-center justify-center rounded-lg border border-[var(--border)] bg-[var(--card)] shadow-sm lg:hidden"
      >
